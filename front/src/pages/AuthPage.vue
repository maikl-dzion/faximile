  ...
  <div class="container">
    ...
    <div class="frame">
      ...
      <div>
        ...
        <a class="btn-goback" value="Refresh" >Go back</a>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    auth() {
      ...
      this.send(apiUrl, "post", this.authData).then(this.authResponse);
    },

    authResponse(response) {

         this.response = response;
         if(!this.response.status || !this.response.jwt_token) {
         ...
         this.store('token'      , this.response['jwt_token']);
         this.store('user_id'    , this.response['user_id']);
         this.store('auth_status', this.response['status']);
    ...
    redirectToPage(role) {
        ...
        switch (role) {
            case 1 :  url = '/'; break;
            case 2 :  url = '/page/performer';  break;
            case 3 :  url = '/page/employee';   break;
            case 4 :  url = '/page/company';    break;
            default:  url = '/page/auth';       break;
        }
...
<style scoped >
  ...
  * {box-sizing: border-box;}
  .transparent {
    ...
    background-image: url(/img/blog-img/home-index.jpg);
